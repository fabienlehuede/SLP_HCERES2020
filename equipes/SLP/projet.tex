...
\tikzstyle{obj} = [rectangle, draw, fill=blue!20, text width=3.5cm, text centered, rounded corners, minimum height=3 em]
\tikzstyle{detail} = [rectangle, draw, fill=plum!20, text width=3.5cm, text centered, rounded corners, minimum height=4 em]
\tikzstyle{detail1} = [rectangle, draw, fill=red!30, text width=3.5cm, text centered, rounded corners, minimum height=4 em]
\tikzstyle{detail2} = [rectangle, draw, fill=orange!30, text width=3.5cm, text centered, rounded corners, minimum height=4 em]
\tikzstyle{detail3} = [rectangle, draw, fill=yellow!30, text width=3.5cm, text centered, rounded corners, minimum height=4 em]
\tikzstyle{detail4} = [rectangle, draw, fill=plum!20, text width=2.5cm, text centered, rounded corners, minimum height=4 em]
\tikzstyle{verrou} = [rectangle, draw, text width=3.5cm, text centered, rounded corners, minimum height=16.5 em]
...
\begin{tikzpicture}[scale=0.5, auto]
    ...
    %  nodes  
    \node [chapeau, text width=7.5cm] (RO) at (12,18) {Domaine de recherche: \;\;\;\; recherche opérationnelle}; 
     \node [chapeau,  text width=8.5cm] (IF) at (29,18) {Toile de fond: industrie du futur};

    \node [obj] (M) at (8,15) {Méthodes};
    \node [obj] (V) at (16,15) {Verrous scientifiques};
    \node [obj] (O) at (24,15) {Objectifs de contribution};
    \node [obj] (D) at (34,-6) {Défis sociétaux};
    \node [obj] (D) at (34,15) {Domaines d'application};
     
   
    \node[detail1] at (8,12) {Optimisation discrète et mixte};
    \node[detail1] at (8,9) {Optimisation exacte};
    \node[detail1] at (8,6) {Méthodes approchées};
    \node[detail1] at (8,3) {Compléxité};
    \node[detail2] at (8,-3) {Optimisation robuste};
    \node[detail2] at (8,-6) {Optimisation stochastique};
    \node[detail3] at (8,-15) {Modélisation avancée};
    \node[detail3] at (8,-18) {Optimisation multiobjectif};
    
     \node[verrou, fill=red!30] at (16,7.5) {Optimisation combinatoire};
    \node[verrou, fill = orange!30] at (16,-4.5) {Incertitude et variabilité};
    \node[verrou, fill=yellow!30] at (16,-16.5) {Modèles Enrichis};
   ...
   \node[detail1] at (24,4.5) {Synchronisation des systèmes};
    \node[detail2] at (24,0) {Systèmes flexibles, reconfigurables};
    \node[detail2] at (24,-3) {Résilience et robustesse des systèmes};
    \node[detail2] at (24,-6) {Réactivité, systèmes dynamiques};
    \node[detail2] at (24,-9) {Optimisation basée sur les données};
    \node[detail3] at (24,-12) {Fonctions objectif alternatives};
    \node[detail3] at (24,-15) {Nouveaux modèles économiques};
    \node[detail3] at (24,-18) {Homme dans la boucle};
    \node[detail3] at (24,-21) {Explicabilité de la décision};
